...
/** \brief La función “ll_filter” recibirá una lista y una función criterio “fn”. Se deberá iterar todos los elementos
...
LinkedList* ll_filter(LinkedList* this, int (*fn)(void*)){
	...
	if (this != NULL && fn != NULL){
		...
		if(nuevaLista != NULL){
			...
			for(i=0;i<len;i++){
				pElementAuxiliar = ll_get(this, i);
				resultado = fn(pElementAuxiliar);
				if(resultado){
					ll_add(nuevaLista, pElementAuxiliar);
				}
			}
		}
	}
...
int ll_count(LinkedList* this, int (*fn)(void* element)){
    ...
    if(this!=NULL && fn != NULL){
    	...
    	for(i=0;i<len;i++){
    		pElementAuxiliar = ll_get (this, i);
    		//printf("\nPrecio obtenido: %f" , acumulador);
    		acumulador = acumulador + fn(pElementAuxiliar);
    		//printf("\nPrecio obtenido subtotal: %f" , acumulador);

        }
    }

   // printf("\nAcumulador retorno: %f" , acumulador);
